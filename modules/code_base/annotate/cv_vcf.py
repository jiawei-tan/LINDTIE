'''
Module      : cv_vcf
Description : Defines VCF object and CrypticVariant object
Copyright   : (c) Jia Wei Tan, Dec 2025
License     : MIT
Maintainer  : https://github.com/jiawei-tan
Portability : POSIX
Defines the VCF and CrypticVariant classes used to generate standardized 
VCF records and to represent cryptic contig-derived structural variants 
within the annotation pipeline. It includes CIGAR operation mappings, 
VCF header/field specifications, and utilities for formatting variant 
information, writing contig summaries, and converting alignments into 
VCF-compatible entries. 
...
class CrypticVariant(object):
    ...
    def write_contig_info(contig_info_file, cv1, cv2=None):
        ...
        is_intx = cv2 and cv1.cvtype in ['FUS', 'IGR'] and cv1.chrom == cv2.chrom
        if is_intx or (cv2 and cv1.cvtype in ['PNJ', 'NEJ', 'AS']):
