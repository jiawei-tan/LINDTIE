...
contigs for novel structural, splicing, and fusion events. It identifies 
insertions, deletions, soft-clips, novel blocks, unannotated junctions, 
and candidate gene fusions; assigns variant types; outputs VCF-formatted 
records; and records detailed contig information. 
...
def annotate_softclips(cv, read, ci_file):
    ...
    for sc_idx in sc_idxs:
        ...
        sc_left = sc_idx == 0
        # Do not immediately force UN; leave classification open if splicing/blocks will annotate later
...
def annotate_fusion(args, read, juncs, bam_idx, ex_ref, ref_trees, outbam):
    ...
    try:
        # Gather all alignments for this contig
        all_reads = [r for r in bam_idx.find(read.query_name)]
        logging.info("Found %d reads for query %s", len(all_reads), read.query_name)
        for idx, r in enumerate(all_reads):
            logging.info("Read %d: reference_name=%s, reference_start=%d, reference_end=%d, is_reverse=%s, cigar=%s",
                        idx, r.reference_name, r.reference_start, r.reference_end, r.is_reverse, r.cigar)
        
        if len(all_reads) == 0:
            logging.info("No alignments found for %s; skipping", read.query_name)
            return
        if len(all_reads) == 1:
            # Single read with soft-clip - process as UN/PNJ/NEJ depending on downstream logic
            logging.info("Found single alignment for %s; processing as single read", read.query_name)
            annotate_single_read(args, all_reads[0], juncs, ex_ref, ref_trees, outbam)
            return
 
        # Select best two segments among >=2 alignments
        # Preference: inter-chromosomal > intra-chromosomal with max genomic distance
        ...
        for i in range(len(all_reads)):
            for j in range(i + 1, len(all_reads)):
                r1, r2 = all_reads[i], all_reads[j]
                key = pair_score(r1, r2)
                if key > best_key:
                    best_key = key
                    best_pair = (r1, r2)
 
        if best_pair is None:
            logging.info("Could not determine a best pair for %s; skipping", read.query_name)
            return
            
        r1, r2 = best_pair
        logging.info("Selected best pair: r1=(%s,%d), r2=(%s,%d)", r1.reference_name, r1.reference_start, r2.reference_name, r2.reference_start)
    except Exception as e:
        logging.info("WARNING: exception while selecting fusion pair for %s: %s", read.query_name, e)
        return

    ci_file = args.contig_info_output
    logging.info("Using contig info output file: %s", ci_file)
    
    cv1 = CrypticVariant().from_read(r1)
    cv2 = CrypticVariant().from_read(r2)
    cv1.cvtype, cv2.cvtype = 'FUS', 'FUS'
    logging.info("Set variant types to 'FUS' for both reads")

    cv1.genes = get_overlapping_genes(r1, ref_trees)
    cv2.genes = get_overlapping_genes(r2, ref_trees)
    logging.info("Overlapping genes for r1: %s", cv1.genes)
    logging.info("Overlapping genes for r2: %s", cv2.genes)
    
    if cv1.genes == cv2.genes and cv1.genes != '':
        # intra-genic rearrangement
        cv1.cvtype, cv2.cvtype = 'IGR', 'IGR'
        logging.info("Both reads overlap the same genes; set variant types to 'IGR'")
    ...
    # --- Process r1: Look for either hard or soft clip, preferring hard ---
    logging.info("Processing r1 for clipping events")
    clip_candidates1 = [(idx, clip[0]) for idx, clip in enumerate(r1.cigar)
                          if clip[0] in (constants.CIGAR['hard-clip'], constants.CIGAR['soft-clip'])]
    logging.info("clip candidates1: %s", clip_candidates1)
    if not clip_candidates1:
        logging.info("WARNING: found no clipping event for read %s; cannot process", read.query_name)
        return
    # Prefer hard clip if available; otherwise, use the first candidate (soft clip)
    clip_idx1, clip_type1 = next(((idx, ct) for idx, ct in clip_candidates1
                                  if ct == constants.CIGAR['hard-clip']), clip_candidates1[0])
    logging.info("Chosen clip for r1: index %d, type %s", clip_idx1, clip_type1)
    clip_left1 = (clip_idx1 == 0)
    logging.info("r1 clip is on the %s", "left" if clip_left1 else "right")

    block_idx1 = 0 if clip_left1 else np.max(np.where(np.array([b[0] for b in cv1.blocks]) < clip_idx1)[0])
    logging.info("Determined block index for r1: %d", block_idx1)
    block1 = cv1.blocks[block_idx1][1]
    cv1.pos = int(block1[0]) if clip_left1 else int(block1[1])
    logging.info("Set breakpoint position for r1 to: %d", cv1.pos)

    # --- Process r2: Look for either hard or soft clip, preferring hard ---
    logging.info("Processing r2 for clipping events")
    clip_candidates2 = [(idx, clip[0]) for idx, clip in enumerate(r2.cigar)
                          if clip[0] in (constants.CIGAR['hard-clip'], constants.CIGAR['soft-clip'])]
    logging.info("clip candidates2: %s", clip_candidates2)
    if not clip_candidates2:
        logging.info("WARNING: found no clipping event for read %s; cannot process", read.query_name)
        return
    clip_idx2, clip_type2 = next(((idx, ct) for idx, ct in clip_candidates2
                                  if ct == constants.CIGAR['hard-clip']), clip_candidates2[0])
    logging.info("Chosen clip for r2: index %d, type %s", clip_idx2, clip_type2)
    clip_left2 = (clip_idx2 == 0)
    logging.info("r2 clip is on the %s", "left" if clip_left2 else "right")

    block_idx2 = 0 if clip_left2 else np.max(np.where(np.array([b[0] for b in cv2.blocks]) < clip_idx2)[0])
    logging.info("Determined block index for r2: %d", block_idx2)
    block2 = cv2.blocks[block_idx2][1]
    cv2.pos = int(block2[0]) if clip_left2 else int(block2[1])
    logging.info("Set breakpoint position for r2 to: %d", cv2.pos)

    # --- Retrieve reference sequences ---
    logging.info("Retrieving reference sequence for r1")
    try:
        refseq1 = r1.get_reference_sequence()
        logging.info("Obtained reference sequence for r1 via get_reference_sequence")
    except Exception as e:
        logging.info("WARNING: Could not retrieve reference sequence for read %s; skipping. Error: %s", r1.query_name, e)
        return

    logging.info("Retrieving reference sequence for r2")
    try:
        refseq2 = r2.get_reference_sequence()
        logging.info("Obtained reference sequence for r2 via get_reference_sequence")
    except Exception as e:
        logging.info("WARNING: Could not retrieve reference sequence for read %s; skipping. Error: %s", r2.query_name, e)
        return

    # --- Process fusion details ---
    logging.info("Processing fusion details")
    # TODO: handle inserted sequence between fusion breakpoints
    varseq1, varseq2 = '', ''
    ...
    logging.info("Set r1 ref to: %s and r2 ref to: %s", cv1.ref, cv2.ref)
    ...
    if r1.is_reverse == r2.is_reverse:
        ...
        logging.info("Fusion details (same strand): cv1.alt: %s, cv2.alt: %s", cv1.alt, cv2.alt)
    else:
        ...
        cv2.alt = ('%s[%s:%d[%s' % (varseq2, cv1.chrom, cv1.pos-1, cv2.ref)) if clip_left2 else \
                  ('%s]%s:%d]%s' % (cv2.ref, cv1.chrom, cv1.pos+1, varseq2))
        logging.info("Fusion details (opposite strands): cv1.alt: %s, cv2.alt: %s", cv1.alt, cv2.alt)

    cv1.vid = get_next_id(r1.query_name)
    cv2.vid = get_next_id(r2.query_name)
    cv1.parid, cv2.parid = cv2.vid, cv1.vid
    logging.info("Assigned variant IDs: cv1.vid: %s, cv2.vid: %s", cv1.vid, cv2.vid)

    # --- Set cpos for cv1 using its chosen clip event ---
    logging.info("Setting cpos for cv1 using chosen clip event")
    ...
    logging.info("Set cpos for cv1 to: %d", cv1.cpos)

    print(cv1.vcf_output())
    print(cv2.vcf_output())
    logging.info("Final VCF output for cv1: %s", cv1.vcf_output())
    logging.info("Final VCF output for cv2: %s", cv2.vcf_output())
    
    outbam.write(r1)
    outbam.write(r2)
    logging.info("Written r1 and r2 to outbam")
    
    CrypticVariant.write_contig_info(ci_file, cv1, cv2)
    logging.info("Wrote contig info to: %s", ci_file)

    # Pass fusion_processed=True to prevent double annotation
    annotate_single_read(args, r1, juncs, ex_ref, ref_trees, genes=cv1.genes, fusion_processed=True)
    logging.info("Annotated single read for r1")
    annotate_single_read(args, r2, juncs, ex_ref, ref_trees, genes=cv2.genes, fusion_processed=True)
    logging.info("Annotated single read for r2")

def _annotate_single_read_debug(args, read, juncs, ex_ref, ref_trees, outbam, genes='', fusion_processed=False):
    '''
    Annotate a single read (not part of a fusion)
    '''
    logging.info("Starting annotate_single_read for read: %s", read.query_name)
    ci_file = args.contig_info_output
    cv = CrypticVariant().from_read(read)
    cv.genes = genes
    logging.info("Set genes for cv: %s", cv.genes)

    # --- Annotate gaps ---
    logging.info("Annotating gaps")
    annotate_gaps(cv, read, ci_file)
    logging.info("Finished annotating gaps")

    # --- Annotate soft clips ---
    logging.info("Annotating soft clips")
    annotate_softclips(cv, read, ci_file)
    logging.info("Finished annotating soft clips")

    # --- Annotate blocks ---
    logging.info("Annotating blocks")
    chr_ref = get_chr_ref(read, ex_ref)
    annotate_blocks(cv, read, chr_ref, ci_file)
    logging.info("Finished annotating blocks")
    ...
    for junc in tx_juncs:
        cv.cvtype = 'PNJ'
        cv.pos = junc[1]
        cv.ref = read.get_reference_sequence()[cv.pos-1]
        cv.alt = read.query_sequence[cv.cpos]
        cv.cpos = sum([v for c,v in read.cigar[:-1] if c in constants.AFFECT_CONTIG])
        cv.vid = get_next_id(read.query_name)
        cv.vsize, cv.cvsize = 1, 1
        CrypticVariant.write_contig_info(ci_file, cv)
        print(cv.vcf_output())
    logging.info("Finished annotating junctions")

    # --- Write to output BAM ---
    logging.info("Writing read to output BAM")
    outbam.write(read)
    logging.info("Finished writing read to output BAM")

    # --- Annotate novel exon junctions ---
    logging.info("Annotating novel exon junctions")
    if not fusion_processed:
        for junc in tx_juncs:
            ...
            CrypticVariant.write_contig_info(ci_file, cv)
            print(cv.vcf_output())
    logging.info("Finished annotating novel exon junctions")

    # --- Annotate alternative splicing ---
    logging.info("Annotating alternative splicing")
    if not fusion_processed:
        for junc in tx_juncs:
            cv.cvtype = 'AS'
            cv.pos = junc[1]
            ...
            CrypticVariant.write_contig_info(ci_file, cv)
            print(cv.vcf_output())
    logging.info("Finished annotating alternative splicing")

    # --- Annotate unknown ---
    logging.info("Annotating unknown")
    if not fusion_processed:
        cv.cvtype = 'UN'
        cv.pos = read.reference_start + 1
        cv.ref = read.get_reference_sequence()[0]
        cv.alt = read.query_sequence[0]
        cv.cpos = 1
        cv.vid = get_next_id(read.query_name)
        cv.vsize, cv.cvsize = 1, 1
        CrypticVariant.write_contig_info(ci_file, cv)
        print(cv.vcf_output())
    logging.info("Finished annotating unknown")

    logging.info("Finished annotate_single_read for read: %s", read.query_name)
...
def _debug_main():
    ...
    logging.info('Starting annotation of contigs')
    logging.info('Loading reference files...')
    ref_trees, ex_trees, ex_ref = get_gene_lookup(args.tx_ref_file)
    juncs, locs = get_junc_lookup(args.junc_file)
    logging.info('Finished loading reference files')

    logging.info('Loading contigs...')
    bam = pysam.AlignmentFile(args.bam_file, 'rc')
    bam_idx = pysam.IndexedReads(bam)
    bam_idx.build()
    logging.info('Finished loading contigs')

    logging.info('Opening output BAM file...')
    outbam = pysam.AlignmentFile(args.output_bam, 'wb', template=bam)
    logging.info('Finished opening output BAM file')

    logging.info('Starting annotation...')
    for read in bam.fetch(multiple_iterators=True):
        logging.info('Processing read: %s', read.query_name)
        if read.is_unmapped:
            logging.info('Read %s is unmapped; skipping', read.query_name)
            continue

        # --- Annotate fusion ---
        logging.info("Annotating fusion for read: %s", read.query_name)
        annotate_fusion(args, read, juncs, bam_idx, ex_ref, ref_trees, outbam)
        logging.info("Finished annotating fusion for read: %s", read.query_name)

        # --- Annotate single read ---
        logging.info("Annotating single read for read: %s", read.query_name)
        _annotate_single_read_debug(args, read, juncs, ex_ref, ref_trees, outbam)
        logging.info("Finished annotating single read for read: %s", read.query_name)

    logging.info('Finished annotation')
    logging.info('Closing output BAM file...')
    outbam.close()
    logging.info('Finished closing output BAM file')
    logging.info('Finished annotation of contigs')
...
def annotate_contigs(args):
    ...
    for read in bam.fetch(multiple_iterators=True):
        ...
        # to skip the contig when query_length is 0
        if qlen == 0:
            logging.info('Skipping contig %s: query_length is 0 (rlen=%d, qlen=%d)' % (read.query_name, rlen, qlen))
            continue
